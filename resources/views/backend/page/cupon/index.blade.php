            ...
            <div class="row">
                ...
                <div class="col-6">
                    <div class="d-flex justify-content-end">
                        <a href="{{ route('coupne.create') }}" class="btn btn-primary"><i class="fa-solid fa-plus"></i> Add
                            New Coupon</a>
                    </div>
                </div>
                <!-- Title End -->
                            ...
                            <tbody>
                                @foreach ($cupon as $key => $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->coupne_name  }}</td>
                                        <td>{{ $item->discount_amount  }}</td>
                                        <td>{{ $item->expire }}</td>
                                        <td>
                                            <button class="btn btn-info"><i class="fa-solid fa-pen-to-square"></i> <a
                                                    href="{{ route('coupne.edit', $item->id) }}"
                                                    class="text-white">Edit</a></button>
                                            <form action="{{ route('coupne.destroy', $item->id) }}" method="post"
                                                id="deleteForm" >
                                                @csrf
                                                @method('DELETE')
